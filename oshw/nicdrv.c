...
int ecx_outframe(ecx_portt *port, int idx, int stacknumber)
{
   ...
   rval = wiznet_macraw_send((*stack->txbuf)[idx], lp);
...
int ecx_outframe_red(ecx_portt *port, int idx)
{
   ...
   {   
      ...
      //ASSERT (0); //HoTam, FIXME
      wiznet_macraw_send(&(port->txbuf2), port->txbuflength2);
...
static int ecx_recvpkt(ecx_portt *port, int stacknumber)
{
   ...
   bytesrx = wiznet_macraw_recv((*stack->tempbuf), lp);
